...
const ClientReviewsCarousel = ({ id }) => {
  ...
  return (
    <>
      {/* {reviews.map((review) => ( */}
      <div className="flex flex-col justify-center items-center gap-5 px-24 sm:px-8 py-14 bg-lightGreen text-white">
        <h1 className="text-[#3F4525] font-bold text-3xl text-center">
          What Our Clients Say
        </h1>
        <p className="text-2xl text-center sm:text-sm ipad:text-xl">
          {reviews[id].review}
        </p>
        <p className="text-center text-lg w-2/3 ">{reviews.review}</p>
        <span className="flex flex-col gap-2 justify-center items-center bg-darkGreen rounded-xl py-5 px-16 w-fit mt-5 text-lg text-center">
